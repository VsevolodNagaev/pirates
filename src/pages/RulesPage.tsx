...
function markdownFor(file: string) {
  const entry = Object.entries(rawModules).find(([k]) => k.endsWith('/' + file) || k.endsWith('\\' + file) || k.includes(file))
...
export function RulesPage() {
  ...
  const bodies = useMemo(() => Object.fromEntries(RULES.map((r) => [r.id, markdownFor(r.file)])), [])

  return (
    <div>
      <p className="text-xs uppercase tracking-[0.28em] text-gold">Правила</p>
      <h1 className="mt-2 font-display text-4xl text-parchment">Как устроен полигон</h1>
      <p className="mt-3 max-w-2xl text-parchment/70">
        Рабочая редакция. Safety и ГИМС важнее игрового текста. Главы открываются вкладками — с
        телефона удобнее, чем вордовский файл.
      </p>

      <Tabs value={tab} onValueChange={(v) => setTab(v as RuleId)} className="mt-8 lg:flex-row lg:items-start">
        <TabsList className="lg:sticky lg:top-20 lg:w-64 lg:flex-col lg:items-stretch">
          {RULES.map((r) => (
            <TabsTrigger key={r.id} value={r.id} className="lg:w-full">
              <span className="mr-2 font-mono text-[11px] text-gold/70">{r.num}</span>
              {r.title}
            </TabsTrigger>
          ))}
        </TabsList>
