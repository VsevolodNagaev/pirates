...
export function RefsPage() {
  ...
  const current = useMemo(() => catalog.find((f) => f.id === faction) as Faction, [faction])

  return (
    <div>
      <p className="text-xs uppercase tracking-[0.28em] text-gold">Костюм</p>
      <h1 className="mt-2 font-display text-4xl text-parchment">Референсы фракций</h1>
      <p className="mt-3 max-w-2xl text-parchment/70">
        Ориентир кроя 1570 года, не обязательный канон. Русский силуэт — только Нарва. Картинки
        отдаются с диска из папки «референсы-фракций» (в репозиторий не входят).
      </p>
        ...
        {catalog.map((f) => (
          <TabsContent key={f.id} value={f.id}>
            ...
            <div className="space-y-10">
              {f.groups.map((g) => (
                <section key={g.title}>
                  <h2 className="mb-4 font-display text-2xl text-gold">{g.title}</h2>
                    ...
                    {g.items.map((item) => (
                      <li key={item.file}>
                        ...
                        >
                          ...
                          />
                          <span className="block px-2 py-2 text-sm text-parchment/85">{item.role}</span>
                        </button>
                      </li>
                    ))}
                  </ul>
          ...
          {open ? (
            <figure className="overflow-hidden rounded-xl border border-gold/30 bg-oak-mid">
              ...
              <figcaption className="flex items-center justify-between gap-3 px-4 py-3">
                ...
                <span className="text-xs text-parchment/50">{current.name}</span>
              </figcaption>
            </figure>
          ) : null}
        </DialogContent>
